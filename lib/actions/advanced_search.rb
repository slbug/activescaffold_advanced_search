    ...
    def do_advanced_search
      ...
      unless @query.empty? && (fields.empty? || modes.empty? || values.empty?)
        ...
        if defined? self.class.create_conditions_for_columns
          conds = merge_conditions(conds, self.class.create_conditions_for_columns(@query.split(' ').map{ |c| like_pattern.sub('?', c) }, columns.select { |c| c.column.text? }, like_pattern)) unless @query.empty?
        else
